...
![Infographic contrasting the slow scripted workflow, where a FeaturePlot takes ninety seconds to render, with the Loupe ecosystem, where the same question is answered in two hundred milliseconds, and showing the .cloupe file as the handoff format between the two](/posts/images/loupe_ecosystem_traditional_vs_loupe_workflow.png)
...
You double click it and it opens. There is no environment to activate, no dependency to resolve, no `renv::restore()` that fails because someone bumped a Bioconductor version. This matters more than it sounds like it should.
