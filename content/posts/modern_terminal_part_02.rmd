...
![Infographic contrasting directory navigation with cd and with zoxide. The upper panel, labelled the old way, shows a long chain of repeated cd ../../ commands running left to right with tangled crossing arrows into scattered folders, annotated no memory, start from zero and tangled paths. The lower panel, labelled the modern way, shows the single command z spatial feeding into a central zoxide intelligence hub scored by frecency, meaning frequency plus recency, backed by a database that learns your destinations, which fans out to the frequent and recent folders you actually work in. Also shown are the interactive fzf picker and the z dash shortcut for going back](/posts/images/modern_terminal_zoxide_frecency_jumping.png)
